# AoC 2022, Day 14: Not Tetris! // Part 2
...
lenx, leny = maxx-minx, (maxy-miny)*2
print(minx, maxx, miny, maxy, lenx, leny)
lines = 0

# Generate an empty wall (of sound!) with 1+2 extra line (our abyss) and 2 extra columns (the fall to abyss)
thewall = [["." for y in range(leny*4)] for x in range(lenx+3)]
#renderwall(thewall)
...
stratae = 0
# Draw the floor(!) at the bottom (of hell!)
for i in range(len(thewall[1])):
    thewall[-1][i] = "#"

# Let's say the sand has to strafe
def sandstrafe(wall, x, y, poststrafe=True):
    #renderwall(wall)
